...
public final class Board {
    ...
    public final @NonNull Tile @NonNull [] @NonNull [] tiles =
            ...
            IntStream.range(0, BOARD_SIZE_TILES)
                    .mapToObj($_ ->
                            IntStream.range(0, BOARD_SIZE_TILES)
                                    .mapToObj($__ -> (@NonNull Tile) new InvalidTile())
                                    .toArray(Tile[]::new))
                    .toArray(Tile[][]::new);
    ...
    public @NonNull String toString() {
        ...
        return MessageFormat.format(
                "Board'{'tiles={0}'}'",
                String.join("\n",
                        (Iterable<String>) Arrays.stream(tiles).map(
                                (row) -> String.join(" ",
                                        (Iterable<String>) Arrays.stream(row)
                                                .map((tile) -> tile.toString().substring(0, 1))
                                                ::iterator)
                        )::iterator
                )
        );
